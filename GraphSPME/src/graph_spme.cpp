 ...
 * j: column
 */
Eigen::SparseMatrix<double> create_bi(Eigen::SparseMatrix<double> &Z, int j) {
  int p = Z.cols();
...
cov_ml(const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &x) {
...
Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>
cov_shrink_spd(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &x) {
  int n = x.rows();
  int p = x.cols();

  // Calculate T_1N and T_2N
  // Calculations may be done without calculating S, see appendix C of Touloumis
  // (2015) Choice depends on p and n
  Dmat centered = x.rowwise() - x.colwise().mean();
  ...
  double T_1N = trS;
  double T_2N = (n - 1.0) * ((n - 1.0) * (n - 2.0) * trS2 + trS * trS - n * q) /
                (n * (n - 2) * (n - 3));
  ...
  Dmat x_square_minus_i =
      x_square.colwise().sum();                 // Notation of code for paper
  ...
  double Y_7N = 2 * (Sum1.cwiseProduct(x_square.colwise().sum()).sum() -
                     (Sum21 + Sum22).sum());
  ...
  double lambda_hat_D = (T_2N + T_1N * T_1N - 2.0 * T_3N) /
...
prec_sparse(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &x,
            Eigen::SparseMatrix<double> &Z, bool cov_shrinkage = true) {
  int n = x.rows();
  int p = x.cols();
  int values_set = 0;
  int si;
